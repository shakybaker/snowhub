...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentBrowserTitle" runat="server">Snowhub : <%= Model.Resort.Name %> Places</asp:Content>
...
<%= Html.ResortNavigation(Model.Resort, "places", Server.HtmlEncode(Request.Url.AbsolutePath))%>
    ...
    <div class="grid_12">
        ...
        <div class="pod">
            ...
            <div class="podbtm" style="height: 35px">
                <div class="cb"></div>
                <a href="/resorts/<%= Model.Resort.PrettyUrl %>/map?m=places" title="" class="smlbutt" style="margin: 5px 5px 5px 10px">Show on Map</a> 
                <div class="cb"></div>
            </div>
        </div>
    </div>
            ...
            if (data.status == "OK") {
                ...
                $.each(data.results, function(i,result){
                    if($.inArray("locality", result.types) < 0)
                    {
                        ...
                        $("#places-target ul").append("<li><a id=\"" + result.reference + "\" href=\"#\" title=\"" + types + "\"><img src=\"" + result.icon + "\" />" + result.name + "</a><span>, " + result.vicinity + "</span></li>");
                        placesFound++;
                    }
                });
                if (placesFound == 0)
