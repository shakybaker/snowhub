...
function addMainResortMarker() {
    ...
    var marker = new GMarker(new GLatLng(document.getElementById("hidLat").value, document.getElementById("hidLng").value), icon);
...
function showMap() {
    ...
    var rid = document.getElementById("hidResortID").value;
...
function addResortMarker(resort, icon) {
    var marker = new GMarker(new GLatLng(resort.Latitude, resort.Longitude), icon);
    map.addOverlay(marker, icon);
    var t;
//    GEvent.addListener(marker, "mouseover", function() {
//        //var myHtml = "<div class='infoWinPanoramio'><img src='/assets/images/logo_panoramio.png' height='20px' width='95px' /><br /><img class='infoWinPhoto' src='" + pic.photo_file_url + "' width='" + pic.width + "px' height='" + pic.height + "px' /><p><strong>Added by:</strong> <a href='#'>username</a></p><strong>Original Image:</strong> <a href='#'>image name</a></p></div>";
...
function getYoutubeMarkers() {
    ...
    scrYoutube.src = 'http://gdata.youtube.com/feeds/api/videos?location=' + document.getElementById("hidLat").value + ',' + document.getElementById("hidLng").value + '&location-radius=10km&alt=json-in-script&callback=storeYoutube';
