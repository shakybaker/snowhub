...
namespace facebook.Linq
{
	...
	public class PropertyData
	{
		...
		internal string GetLinqFieldName()
		{
			...
			if(PropertyInfo.PropertyType==typeof(DateTime) && PropertyInfo.Name.EndsWith("_date"))
			{
				var originalName = PropertyInfo.Name.Substring(0, PropertyInfo.Name.Length-5);
				if (PropertyInfo.DeclaringType.GetProperty(originalName) != null)
				{
					return originalName;
				}
			} 
	...
	internal static class Helper
	{
		...
		{
			if (pi.DeclaringType.IsSubclassOf(typeof(XTypedElement)) && IsSupported(pi.PropertyType))
			{
				if(pi.GetSetMethod()!=null || pi.PropertyType==typeof(DateTime))
					return true;
			}
			return pi.GetCustomAttributes(typeof(ColumnAttribute), true).Length > 0;
		}

		static HashSet<Type> SupportedTypes = new HashSet<Type> { typeof(Int64), typeof(Decimal), typeof(string), typeof(bool), typeof(DateTime), typeof(int) };
